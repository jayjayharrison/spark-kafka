https://medium.com/@thinh.onblurvn/some-helpful-exploration-functions-in-panda-to-spark-dataframe-7694c6f5bbd9

gratefully learned from Thinh Duong's post
...
object DataFrameExtensions {
  ...
  class DataFrameStructVisitor extends StructTypeVisitor[DataFrame] {
    ...
    override def visitValue(df: DataFrame): DataFrame = df.withColumn(s"${dfname.replace('.','-')}", F.col(s"$dfname"))
  }
    ...
    field.dataType match {
      ...
      case StructType(subFields) => {
        ...
        val df_tmp = subFields.foldLeft(df)((df_acc, subField) => {
          val key = (prefixes :+ field.name :+ subField.name).mkString(".")
